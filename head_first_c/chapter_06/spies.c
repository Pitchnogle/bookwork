...
main()
{
	char question[80];
	char suspect[20];
	node *start_node = create("Does suspect has a mustache");
	start_node->no = create("Loretta Barnesworth");
	...
	do {
		...
		while (1) {
			if (yes_no(current->question)) {
				...
				printf("Give me a question thst is TRUE for %s but not for %s? ",
					suspect, current->question);
				fgets(question, 80, stdin);

				// The bug-fix -- the old memory leak is because this wasn't free'd
